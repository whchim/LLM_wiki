...
**当前状态**：Phase 2 已交付（SP1 PostgreSQL 迁移 / SP2 FastAPI+JWT 认证 / SP2.5 可观测 / SP3 watcher 全自动编译 / SP4 混合检索 / SP5 健康巡检）；多租户路线图 L2（队列）/L3（RLS）/L3.5（文件分区）/L4（模型配置与用量）与 **L5 应用内问答（对话窗口）+ 知识图谱** 已交付；销售 Agent 阶段 0-5 已交付（含 Vue 3 工作台、转人工处置闭环、状态建议规则+模型双引擎）。**pytest 收集 454 个用例** + CI（测试 + Prompt 退化检测 + 检索合成集门禁）+ LLM 输出契约校验。
...
| [docs/WIKI-90_知识图谱_设计文档.md](docs/WIKI-90_知识图谱_设计文档.md) | **知识图谱**：从 Markdown 派生节点/边、**保留文件（索引/日志/规范）是一等节点**、待建页面（红链）即知识缺口、Obsidian 同款 d3-force + canvas 渲染 | 动 `/graph`、`core/graph.py`、图谱页或 `log.md` 前 |
...
- **知识图谱（`core/graph.py` + `/graph`，v0.2）**：从 Markdown **派生**（不新增权威表：文件仍权威，图谱是可重建视图）。节点 = 已发布条目 + **保留文件**（`index.md`/`log.md`/`SCHEMA.md`，`kind=index/log/schema`、`is_meta`）+ 可选 `pending_review/`、`RAW/`（`kind=raw`）；边 = frontmatter `related_to` + 正文 `[[wikilink]]` + `[文字](路径.md)`；解析顺序：目录相对 → 根相对 → 标题/主干 → **类型前缀**（`[[概念-新质生产力]]`）→ 末段兜底（真机语料逼出来的四层，见 `docs/WIKI-90` §7.1）。**被引用但解析不到的 = 待建页面（红链）**，带引用次数与来源——比"搜不到"更精确的知识缺口。⚠️ **保留文件是一等节点、不隐藏**（PRD 要求；v0.1 曾把它们排除，用户指出与编译范式不符）：索引度数最高是**事实**，交前端着色/过滤。外链与自链接不进图；`_triggers/`/`_suggestions/`/`tenants/` 不进图；`max_nodes` 超限 `truncated=true` 如实标注；按租户分区。前端「知识图谱」页 = **d3-force + canvas**（Obsidian 同源技术栈）：光标中心滚轮缩放、拖拽平移、拖节点固定/释放、**悬停高亮 1 跳邻域**并淡出其余、物理旋钮（中心/斥力/连线力/连线距离/向心力）、按 `kind` 过滤、搜索定位、适应视图；待建页面是虚线空心节点。**视觉与布局别靠猜**：改参数前跑 `cd frontend && node tools/graph_snapshot.mjs`（用同一套参数渲染真实数据 + 打印连通分量/重叠/包围盒指标，再用无头 Edge 截图看），三条经验已固化在代码注释里——**边线/画布配色必须跟随主题**、**斥力必须带 `distanceMax` 否则多分量图会摊成"星尘"**、**标签用预算制而不是度数阈值**（215 边/109 节点时"度≥4"几乎人人命中）。⚠️ **渲染状态必须用 `shallowRef`**：`simNodes` 曾用普通 `let` 数组，而 `visibleNodes` 是由它派生的 computed——模板首屏（数据未到）读一次就把"空数组"永久缓存，真机上表现为**画布全空而统计行还显示"节点 0 · 关系 215"**（单测与快照都抓不到）。**改完必须跑页面 E2E 冒烟**：`cd frontend && npm run smoke`（真登录 + 打开图谱页 + 对 canvas 采样非背景像素占比 + 收集控制台错误 + 截图；用系统 Edge，不额外下浏览器）
- **保留文件 `NEXUS/log.md`（PRD WIKI-00 §Reserved Files）**：`index.md`（渐进式目录）由编译链路逐次更新；**`log.md` 是操作审计日志**，由 `ops.append_log()` 从**编译（成功/失败）、审核（放行/驳回/重提）、上传**三条链路 append-only 写入（文件侧镜像 `audit_logs`/`trace_events`）。⚠️ 该文件长期是 `init.sh` 建的空占位、**没有任何代码写它**——真机核查发现 0 字节，属"PRD 要求但未实现"，已补齐（`docs/WIKI-90` §6.1、`tests/test_log_reserved_file.py`）
