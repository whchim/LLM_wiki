...
1. 对每个条目：
   ...
   b. 构造去重候选：`grep -l "<标题关键词>" ${KB_ROOT:-vault}/NEXUS ${KB_ROOT:-vault}/pending_review --include=*.md`（排除自身），每个候选取路径+标题+前200字，最多 5 个
   c. 并行执行（Harness parallel）：
      - 确定性检查（bash/python，不调 LLM）：
        - 维度1 完整性 + 维度5 敏感信息：调用确定性规则单一权威实现 `core/rules.py`：
          `python -c "import sys; sys.path.insert(0, 'core'); from rules import check_completeness, check_sensitive; ..."`
   ...
   e. 写入审核结果（仅契约校验通过后）：若该 nexus_path 已有 human_decision IS NULL 的行则 UPDATE 该行（按 id 更新 ai_verdict/ai_scores），否则 INSERT。调用 `core/db.py` 的
      `insert_review(nexus_path, submitter, department, ai_verdict, ai_scores)`（python -c 内联：
      `import sys; sys.path.insert(0, 'core'); from db import insert_review; insert_review(...)`），不手写字段列表
2. 触发文件处理完毕移入 ${KB_ROOT:-vault}/_triggers/done/
3. 返回：审核 N 条、判定分布
