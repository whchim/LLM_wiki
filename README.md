...
[![tests](https://img.shields.io/badge/tests-398%20passed-green)]()
...
**Vue 3 工作台**（`frontend/`，Vue 3 + Element Plus + Vite）是**唯一界面**，覆盖知识域（总览 / 我的知识库 / 全部条目 / **对话窗口** / **知识图谱** / 上传 / 审核）与销售域（销售澄清 / 客户状态）；生产由 nginx 托管构建产物并反代 `/api`，开发态走 Vite 代理，共用同一套 FastAPI 权限、审计与状态机边界。

**应用内问答（对话窗口）**：`POST /ask` 检索知识库 → 只依据**已发布条目**作答，每条结论带**可点开核对的引用**（引用路径必须来自本次检索结果、原文片段必须逐字可查，代码校验，违例回灌重试一次后仍违例就明确失败）。**检索不到就不答**（不调用模型，如实说"知识库暂无"），并把这次提问记成知识缺口进自增长看板——`Compile-time RAG` 的消费端由此闭环。

**知识图谱**：`GET /graph` 从编译产物派生节点与关系（`related_to` + `[[wikilink]]` + 链接），把**被引用却尚未建立的条目**列成待建清单（wiki 的"红链"）——比"搜不到"更精确的知识缺口，直接给出该补哪一页。**知识库保留文件（`index.md` 渐进式目录 / `log.md` 操作日志 / `SCHEMA.md` 规范）是一等节点**：按 PRD WIKI-00 §Reserved Files，它们是知识库自我描述的一部分。前端图谱页与 Obsidian 图谱同源：**d3-force 物理 + canvas 渲染**，滚轮缩放、拖拽平移、拖节点固定、**悬停高亮邻域并淡出其余**、物理参数旋钮、按类型过滤，待建页面以虚线空心节点呈现。
...
**知识浏览**：工作台的「全部条目」页可在线预览 Markdown 正文，「知识图谱」页查看关系与待建页面；要看 Obsidian 原生图谱与反向链接，用 [Obsidian](https://obsidian.md/) 打开 `vault/` 目录。
...
| [`docs/WIKI-70_Phase2_应用内编译引擎_设计文档.md`](docs/WIKI-70_Phase2_应用内编译引擎_设计文档.md) | 应用内编译/审核引擎 + 多租户路线图：L2 队列 / L3 RLS / L3.5 文件分区 / L4 模型配置与用量进库 |
| [`docs/WIKI-80_对话窗口_设计文档.md`](docs/WIKI-80_对话窗口_设计文档.md) | 应用内问答（L5）：门禁先于模型（无依据不答）/ 引用溯源契约 / 缺口回流 / 检索原语抽取 |
| [`docs/WIKI-90_知识图谱_设计文档.md`](docs/WIKI-90_知识图谱_设计文档.md) | 知识图谱：从 Markdown 派生关系（`related_to` + wikilink）/ **保留文件是一等节点** / **待建页面即知识缺口** / Obsidian 同款 d3-force + canvas |
| [`docs/VAL-01_LLM_输出校验_设计说明.md`](docs/VAL-01_LLM_输出校验_设计说明.md) | LLM 输出契约校验：四组 schema + 门禁进 agent loop + 退化检测 |
...
- **454 个 pytest 用例**：DDL 幂等、双写一致性、审核规则边界（中文紧邻漏报/金额阈值）、上传批处理补偿、驳回重提流程、索引重建鲁棒性、JWT 鉴权与越权、审计落库、搜索缺口聚合、启动自愈、pgvector 混合检索与降级、缺口判据、LLM 输出契约校验、LLM 可观测上报边界，以及多租户（队列并发认领/租约自愈、RLS 读写隔离、**文件按租户分区**、租户模型配置与配额、条目身份按租户）、应用内问答（无依据不调模型、引用逐字溯源、契约违例回灌、缺口回流）、知识图谱（三类边、四层链接解析、**保留文件一等节点**、待建页面计数、外链过滤、租户隔离）、**保留文件 `log.md` 的 append-only 写入**，以及应用层的状态转移约束、敏感数值分层、澄清轮次预算、提交内容级幂等、状态建议双引擎、合成回放评测
...
├── tests/                # 454 个 pytest 用例（连真实 PostgreSQL 隔离库；含可公开合成检索语料）
```
...
- **Phase 3 规划**：图谱的实体/关系抽取与中心性算法（现已交付**服务端图谱 + Obsidian 同款交互**，缺的是抽取增强与图算法）、多租户 RBAC、外部源感知、分布式编译、生产级部署（详见 PRD）
