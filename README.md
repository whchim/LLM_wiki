...
**知识图谱**：`GET /graph` 从编译产物派生节点与关系（`related_to` + `[[wikilink]]` + 链接），并把**被引用却尚未建立的条目**列成待建清单（wiki 的"红链"）——比"搜不到"更精确的知识缺口，直接给出该补哪一页。前端图谱页用**自研 SVG 力导向布局**（不引第三方大包）：悬停高亮邻域、点节点看原文、右侧列待建页面与孤立条目。
...
| [`docs/WIKI-90_知识图谱_设计文档.md`](docs/WIKI-90_知识图谱_设计文档.md) | 知识图谱：从 Markdown 派生关系（`related_to` + wikilink）/ **待建页面即知识缺口** / 自研力导向布局 |
...
├── tests/                # 398 个 pytest 用例（连真实 PostgreSQL 隔离库；含可公开合成检索语料）
```
