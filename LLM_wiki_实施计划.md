...
- 路径约定：Vault 相对路径（如 `NEXUS/概念/示例监测产品.md`）为知识条目唯一标识；代码内不写绝对路径，统一从 `KB_ROOT`/`DB_PATH` 环境变量取（容器内 `/app/vault`）
...
- 种子数据依赖：真实示例企业文档由用户提供（设计文档 9.2），本计划仅含 3 份测试样例
...
def test_review_insert_and_list(fresh_db):
    m = fresh_db
    rid = m.insert_review("pending_review/示例监测产品.md", "demo_user", "产品", "approved",
                          '{"verdict":"approved"}')
...
```bash
cd "d:\桌面\LLM_wiki" && sqlite3 vault/meta.db "INSERT INTO pending_reviews (nexus_path, submitter, department, ai_verdict, ai_scores, created_at) VALUES ('pending_review/示例监测产品.md','demo_user','产品','approved','{\"verdict\":\"approved\",\"scores\":{\"completeness\":\"pass\",\"dedup\":\"pass\",\"quality\":4,\"sensitive\":\"pass\",\"compliance\":\"pass\"},\"department\":\"产品\",\"concerns\":[],\"summary\":\"质量合格\"}','2026-08-13');" && bash -c 'echo "---" > vault/pending_review/示例监测产品.md && echo "type: concept" >> vault/pending_review/示例监测产品.md && echo "title: 示例监测产品" >> vault/pending_review/示例监测产品.md && echo "status: pending" >> vault/pending_review/示例监测产品.md && echo "---" >> vault/pending_review/示例监测产品.md && echo "" >> vault/pending_review/示例监测产品.md && echo "# 示例监测产品" >> vault/pending_review/示例监测产品.md && echo "示例企业的多灾种监测预警产品。" >> vault/pending_review/示例监测产品.md'
```
...
```bash
cd "d:\桌面\LLM_wiki" && sqlite3 vault/meta.db "INSERT INTO search_logs (query, match_count, source, timestamp) VALUES ('区块链',0,'streamlit',datetime('now','localtime')),('区块链',0,'claude_code',datetime('now','localtime')),('示例监测产品',3,'streamlit',datetime('now','localtime'));"
```
...
**Files:**
- Create: `tests/sample_docs/样例_示例监测产品产品白皮书.md`
...
`tests/sample_docs/样例_示例监测产品产品白皮书.md`（500+ 字，含 2 个可提取概念）：

```markdown
# 示例监测产品产品白皮书

## 产品概述
示例监测产品是示例企业自主研发的多灾种监测预警产品，面向应急管理部门提供暴雨、台风、地质灾害等灾害的实时监测与预警服务。产品采用"感知-分析-决策-行动"四层架构，覆盖监测数据接入、智能分析研判、预警信息发布、应急响应闭环全流程。
...
cd "d:\桌面\LLM_wiki"
# 种子 search_logs（看板演示）
sqlite3 vault/meta.db "INSERT INTO search_logs (query, match_count, source, timestamp) VALUES ('区块链技术方案',0,'streamlit',datetime('now','localtime')),('示例监测产品报价',0,'streamlit',datetime('now','localtime')),('应急预案模板',0,'claude_code',datetime('now','localtime'));"
# 上传样例文档触发编译队列
cp tests/sample_docs/样例_示例监测产品产品白皮书.md vault/RAW/产品资料/ 2>/dev/null || mkdir -p vault/RAW/产品资料 && cp tests/sample_docs/样例_示例监测产品产品白皮书.md vault/RAW/产品资料/
```
