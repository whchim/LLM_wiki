...
type: concept                              # 必填
title: "应急哨兵"                            # 推荐
description: "华泰智远自主研发的多灾种监测预警产品"
tags: ["产品", "应急管理", "监测预警"]
...
2. **路径即主键**：知识条目的唯一标识是其在 Vault 中的相对路径（如 `NEXUS/概念/应急哨兵.md`），不使用自增 ID。wikilink 天然引用路径，消除 ID→路径 的转换层。
...
> **为什么 knowledge_entries 用 path 做主键而不是自增 ID**：Obsidian wikilink `[[概念-应急哨兵]]` 直接对应文件路径 `NEXUS/概念/应急哨兵.md`。用 ID 做主键意味着每次 wikilink 解析都要 `SELECT path FROM entries WHERE id=?`——纯浪费。路径即身份。
...
| 种子数据 | 5-10 份华泰智远真实文档预置在 RAW 目录 | P1 | |
...
Demo 需要 5-10 份华泰智远真实文档作为种子数据。这些文档被预置在 `data/RAW/` 目录下，启动时已被系统发现并处于"待编译"状态。
...
|------|------|
| **数量** | 5-10 份 |
| **主题** | 涵盖 2-3 个核心产品/业务线（如应急哨兵、叫应体系等） |
| **领域覆盖** | 至少涉及 2-3 个部门（如产品 + 售前 + 开发） |
| **格式** | 以 .md 为主（PDF/DOCX 需额外提供文本提取后的版本用于对照） |
| **关联性** | 至少 2 对文档包含可互链的知识点（如两份文档都提到"应急哨兵"，可从不同角度互相引用） |
| **长度** | 每份 500-5000 字 |
...
| # | 模拟文件名 | 类型 | 部门 | 包含的知识概念 |
|---|-----------|------|------|-------------|
| 1 | 应急哨兵产品白皮书.md | 产品文档 | 产品 | 应急哨兵、多灾种监测预警、监测预警平台 |
| 2 | 叫应体系技术方案.md | 技术方案 | 售前 | 叫应体系、应急响应流程、多级联动 |
| 3 | 应急哨兵部署手册.md | 实施交付 | 实施交付 | 部署架构、数据接入规范、运维监控 |
...
[02:30] 切换回 Obsidian
        NEXUS/概念/ 下出现 应急哨兵.md、多灾种监测预警.md 等新概念页
        打开应急哨兵.md → 渲染完整的 Markdown 知识页面
