...
- **Reserved Files**：保留 `index.md`（渐进式目录，编译时逐次更新）和 `log.md`（审计日志，Demo 创建空文件占位，Phase 2 起记录操作事件）
  - **实现状态（2026-09-15 核查）**：`index.md` 由编译链路逐次更新 ✅；`log.md` **此前一直是空文件占位**——
    真机检查发现 0 字节、没有任何代码写它，本条要求实质未落地。现已补齐：`core/ops.append_log()`
    由编译（成功/失败）、审核（放行/驳回/重提）、上传三条链路 append-only 写入（`docs/WIKI-90` §6.1）。
  - 两个保留文件在**知识图谱里是一等节点**（`kind=index/log`）：它们是知识库自我描述的一部分，不隐藏。
...
| 政策 | 共享层为主 | 国家/地方政策与规划纲要，跨部门通用 |
| 行业研究 | 由研究领域判定 | 行业报告/市场分析/竞品研究 |
| 网络文摘 | 共享层为主 | 网络文章、百科、新闻转载摘录 |
| 论文 | 由研究方向判定 | 学术论文、技术白皮书 |
| 客户沟通 | 销售/售前 | 客户需求、客户问答记录、市场答疑 |

> **v2.4 修订**：来源类型原为 4 类（个人_notes/会议/经验/项目，Demo 期口径）。接入真实企业语料后，
> 政策/行业研究/网络文摘/论文/客户沟通 类文档在编译契约（`core/output_schema.SOURCE_TYPES`）
> 被判非法、重试仍不通过（实测：政策类文档模型坚持输出"政策规划"），故补齐来源类型枚举。
