...
> **一次洽谈 = 一个会话（内容级幂等）**：同一客户可以有多次洽谈，每次洽谈各自一个澄清会话。
> 但**同一份纪要重复提交不算新洽谈**——`POST /clarifications/intake` 按"同客户 + 相同正文指纹"
> 复用原洽谈与会话（响应带 `duplicate`），避免销售看到一堆看起来一模一样的记录；
> 确实要再谈一次需显式 `force_new=true`。幂等键（`idempotency_key`）只防"同一个键重放"，
> 前端每次读文件都会换键，因此**内容指纹才是防重复提交的实际闸门**。
