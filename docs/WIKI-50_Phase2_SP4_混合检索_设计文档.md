...
- **v0.1.3（2026-09-14）**：把检索回归门禁**搬进 CI**（原状：真实黄金集是本地/面试资产、不进公开仓库，CI 因此完全不跑门禁——"改动检索无人在 CI 拦"）。新增可公开的**合成评测集** `tests/fixtures/retrieval_kb/`（6 个 `status: active` 页面 + 1 个 `status: draft` 页面，通用企业流程内容）与 `tests/fixtures/retrieval_gold.md`（11 条：精确 6 / 语义 2 / 缺口 3）；`tools/eval_search.py` 新增 `--kb` / `--gold` / `--no-vector`，`main()` 改为返回退出码（可被测试 in-process 调用），缺口行的"（零命中）"占位归一为空列表。CI 步骤 `--check --no-vector`（合成页面对应不到 PG 向量索引，故强制关向量、走 grep 降级）：断言缺口判据 3/3、精确组 Recall@10 不回退。新增 `tests/test_eval_search_gate.py`（6 例）：门禁在合成集上通过、**标注写错时确实会拦**（mutation 式自检）、黄金集缺失即失败、draft 页不参与检索、`--no-vector` 降级正确、黄金集与语料互相锁定（路径存在 + 精确项 grep 必中 + 缺口项零命中）。真实黄金集与向量/融合通道的完整评测仍走本地 `python tools/eval_search.py --check`。
